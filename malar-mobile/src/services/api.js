...
// For physical device testing or production, use the deployed backend URL
const API_BASE = process.env.EXPO_PUBLIC_API_URL || 'https://malarxerox-1.onrender.com';

// Helper to get token from secure storage
const getToken = async () => {
  try {
    const saved = await SecureStore.getItemAsync('malar_auth');
    if (saved) {
